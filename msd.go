...
func msd(r *list, n int) (res *list) {
	...
	for len(stack) > 0 {
		...
		stack = intoBuckets(stack, s.head, s.pos)
	}
	return
}
...
// in position pos. We keep track of the minimum and maximum characters
// encountered. In this way we may avoid looking at some empty buckets
// when we traverse the buckets in order and push the lists onto the stack.
func intoBuckets(stack []frame, a *list, pos int) []frame {
	b := make([]bucket, 257) // b[256] holds strings shorter than pos.
	chMin, chMax := 255, 0
